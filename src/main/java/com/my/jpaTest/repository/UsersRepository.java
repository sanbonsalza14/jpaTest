...
//    List<Users> findByEmailContains(String x);
    ...
    //    or(gender= 'Female' and name like '%M%')
    List<Users> findByGenderAndNameContainingIgnoreCaseOrGenderAndNameContainingIgnoreCase(
            Gender gender1, String keyword1,
            Gender gender2, String keyword2
    );

    // 문제 2. 이메일에 net을 포함하는 데이터 건수를 출력하시오.
    //select count from users where email like %net%)
    List<Users> findByEmailContains(String keyword);

    //문제 3. 문제 3. 가장 최근 한달이내에 업데이트된 자료 중 이름 첫자가 "J"인 자료를 출력하시오.
    //select from users where updatedAt >= :startDate AND name like concat(:refix, '%'))
    List<Users>findByUpdatedAtGreaterThanEqualAndNameStartingWith(
            LocalDateTime startDate,
            String prefix
    );

    //문제 4. 가장 최근 생성된 자료 10건을 ID, 이름, 성별, 생성일 만 출력하시오.
    // select id,name,gender,createdAt from users ORDER BY createdAt desc;
    List<Users> findTop10ByOrderByCreatedAtDesc();

    //문제 5. "Red"를 좋아하는 남성 이메일 계정 중 사이트를 제외한 계정만 출력하시오.
    //(예, apenley2@tripod.com  → apenley2)
    //select substring(email,1,locate('@', u.email)-1)
    //from users where gender = :gender and likeColor = :color and locate('@', email) >0
    List<Users> findByGenderAndLikeColor(Gender gender, String color);
}